        ...
        cards.slice(0, 4).forEach((card) => {
          ...
          cardDiv.addEventListener("click", function (event) {
            ...
            if (!enemyMoveset.querySelector(moveID)) {
              ...
              enemyMoveDefense.innerHTML =
                "Defend: " + enemyTurn["move_defend"];
            ...
            fetch("getRoundResult.php", {
              // pushing into this api so that backend can do calc
              method: "POST",
              headers: {
                "Content-Type": "application/json",
              },
            ...
            takenEnemyDamage =
              enemyTurn["move_defend"] - currentRound["card_attack"]; // -
            ...
            if (
              enemyHealthBar.value + roundEnemyEnergy >
              currentEnemy["enemy_energy"]
            ) {
              enemyHealthBar.value = currentEnemy["enemy_energy"];
              enemyHealthNum.innerHTML =
                currentEnemy["enemy_energy"] +
                "/" +
                currentEnemy["enemy_energy"];
            } else {
              ...
              enemyHealthNum.innerHTML =
                newEnemyHealthValue + "/" + currentEnemy["enemy_energy"];
            }
            ...
            document.querySelectorAll(".enemyMove").forEach((elem) => {
              if (
                elem.querySelector("div").innerHTML === enemyTurn["move_name"]
              ) {
                elem.classList.add("focus-b");
              } else elem.classList.remove("focus-b");
            });
          ...
          (cardImage.className = "cardpic"),
            (cardImage.src = "../media/cards/" + card["card_img"]);
