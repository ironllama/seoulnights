  ...
  fetch(`getEncounterData.php`, {
    ...
    .then((visitsLeft) => {
      if (visitsLeft <= 0) {
        ...
        document
          .querySelector(".open-store-button")
          .classList.remove("focus-e");
      }
    ...
    .then((data) => {
      ...
      data.items["drink"].forEach((drink) => {
        ...
        drinkButton.innerHTML =
          drink["item"] +
          "<br>" +
          Math.abs(drink["price_hit"]).toLocaleString("en-US");
        drinkButton.id = "s" + drink["mart_id"];
        drinkButton.style.backgroundImage =
          "url(../media/mart/" + drink["item_img"];
        if (
          parseInt(moneyNum.innerHTML.replace(/,/g, ""), 10) >
          drink["price_hit"]
        ) {
      ...
      data.items["food"].forEach((food) => {
        ...
        foodButton.innerHTML =
          food["item"] +
          "<br>" +
          Math.abs(food["price_hit"]).toLocaleString("en-US");
        foodButton.id = "s" + food["mart_id"];
        foodButton.style.backgroundImage =
          "url(../media/mart/" + food["item_img"];
        if (
          parseInt(moneyNum.innerHTML.replace(/,/g, ""), 10) > food["price_hit"]
        ) {
