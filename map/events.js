...
const optionDescriptionArray = Array.from(
  document.querySelectorAll(".option-description")
);
const optionEnergyArray = Array.from(
  document.querySelectorAll(".option-energy")
);
...
function triggerEvent(inData) {
  ...
  eventZone.querySelector(
    ".event-image"
  ).style.backgroundImage = `url("../media/events/${inData.event_img}")`;
  eventZone.querySelector(".prompt-container").innerHTML =
    inData.event_description;
  ...
  if (inData.options[0].option_money + currentMoney >= 0)
    optionButtons[0].addEventListener("click", sendChoice);
  else {
    optionButtons[0].innerHTML =
  ...
  if (inData.options[1].option_money + currentMoney >= 0)
    optionButtons[1].addEventListener("click", sendChoice);
  else {
    optionButtons[1].innerHTML =
      '<svg xmlns="http://www.w3.org/2000/svg" height="16" width="16" viewBox="0 0 512 512"><path d="M367.2 412.5L99.5 144.8C77.1 176.1 64 214.5 64 256c0 106 86 192 192 192c41.5 0 79.9-13.1 111.2-35.5zm45.3-45.3C434.9 335.9 448 297.5 448 256c0-106-86-192-192-192c-41.5 0-79.9 13.1-111.2 35.5L412.5 367.2zM0 256a256 256 0 1 1 512 0A256 256 0 1 1 0 256z"/></svg>';
  ...
  if (inData.options[2].option_money + currentMoney >= 0)
    optionButtons[2].addEventListener("click", sendChoice);
  else {
    optionButtons[2].innerHTML =
      '<svg xmlns="http://www.w3.org/2000/svg" height="16" width="16" viewBox="0 0 512 512"><path d="M367.2 412.5L99.5 144.8C77.1 176.1 64 214.5 64 256c0 106 86 192 192 192c41.5 0 79.9-13.1 111.2-35.5zm45.3-45.3C434.9 335.9 448 297.5 448 256c0-106-86-192-192-192c-41.5 0-79.9 13.1-111.2 35.5L412.5 367.2zM0 256a256 256 0 1 1 512 0A256 256 0 1 1 0 256z"/></svg>';
  ...
  function sendChoice(event) {
    // nested function - sending the options data to be tracked once clicked
