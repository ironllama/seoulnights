...
function youLose() {
  ...
  resolutionText.innerHTML =
    "You are out of money and too broke to go on! You couldn't handle a night out in Hongdae! Try again?";
...
function showResults(inData) {
  ...
  if (gameRound < cardZoneList.length - 1) {
    narrationTypewriter(
      `It is now ${formatAsOclock(gameRound)}. Your night continues...`
    );
  ...
  if (inData["updatedEnergyLevel"] <= 0) {
    ...
    resolutionText.innerHTML =
      "You are out of energy and couldn't handle a night out in Hongdae! Try again?";
    document.querySelector(".state-changes-container").innerHTML = "";
    encounterResult.addEventListener("click", () => {
      window.location.reload();
    });
